...
class SimulationEngine:
    ...
    def __init__(self, models: Dict[str, GarchModel], copula: CopulaModel, drift_overrides: Dict[str, float] = None):
        self.models = models
        self.copula = copula
        self.drift_overrides = drift_overrides if drift_overrides else {}
    ...
    def generate_scenarios(self, n_scenarios: int, n_steps: int) -> pd.DataFrame:
        ...
        # generate correlated Uniforms for all steps and scenarios
        # total samples needed = n_scenarios * n_steps
        total_samples = n_scenarios * n_steps
        uniforms_flat = self.copula.sample(total_samples)
        ...
        for i, asset in enumerate(self.asset_names):
            ...
            alphas[i] = p['alpha[1]']
            betas[i] = p['beta[1]']
            mus[i] = self.drift_overrides.get(asset, p['mu'])
        ...
        # pre-calc z-scores (expensive ppf) outside loop
